...
-- DDL modelo de datos ---------------------------------------------------------------
-- Definción de la estructura de la base de datos
-- todas las restricciones se deben definir usando la instrucción CONSTRAINT
-- -------------------------------------------------------------------------------------
...
-- Tabla viaje
-- Tabla de viaje modificada para evitar el error de dupliación de nombres de columnas
-- se mantiene el diseño extraido del ERDPlus
CREATE TABLE viaje
(
  fecha INT NOT NULL,
...
evitando así que un viaje tenga la misma sucursal como origen y destino.
*/

ALTER TABLE chofer
	ADD CONSTRAINT UQ_chofer_dni UNIQUE (dni);
-- UNIQUE (dni): Garantiza que los valores en la columna dni sean únicos, evitando duplicados.
...
-- Insertar datos en la tabla camion_producto (productos transportados por cada camión)
...
-- ATENCIÓN ---------------------
-- ------------------------------
-- Caso inconsistente donde se registran camiones diferentes (Error en el diseño)
INSERT INTO viaje (fecha, id_viaje, duracion, distancia, peso_transportado, id_producto, id_camion_a, id_sucursal_origen, id_sucursal_destino, id_camion_b, id_chofer) VALUES
(20230902, 2, 3, 700, 20, 2, 2, 1, 3, 3, 3); 
-- `id_camion_a` es 2 y `id_camion_b` es 3, lo cual es inconsistente

-- Instrucción SELECT que visualiza todos los registros de la tabla
...
-- ------------------------------------------------------------------------------
-- CORRECCIÓN DEL SCRIPT PARA AJUSTAR EL MODELO
-- -----------------------------------------------------------------------------

-- Ajuste del modelo para validar la integridad de los datos
-- Se debe asegurar que sea el mismo camión para producto como para chofer
...
-- Se verifica que se eliminó la situación de inconistencia
INSERT INTO viaje_nueva (fecha, id_viaje, duracion, distancia, peso_transportado, id_producto, id_camion, id_sucursal_origen, id_sucursal_destino, id_chofer) VALUES
(20230902, 2, 3, 700, 20, 2, 2, 1, 3, 2); 
-- el número de camión es igual tanto para producto y chofer
...
-- -------------------------------------------------------
-- VERIFICACIÓN DE RESTRICCIONES DEFAULT, UNIQUE, CHECK --
-- -------------------------------------------------------

-- Insert inválido en la tabla viaje (Sucursales de origen y destino son iguales, violando la restricción CHECK)
-- error debido a la restricción CK_viaje_sucursal_origen_destino
...
-- Insert válido en la tabla chofer (DNI único)
INSERT INTO chofer (id_chofer, nombre, apellido, dni, id_sucursal, id_categoria_lic)
VALUES (4, 'Luis', 'Martinez', 45678901, 2, 1);

-- Insert inválido en la tabla chofer (DNI duplicado, violando la restricción UNIQUE)
-- error debido a la restricción UQ_chofer_dni
INSERT INTO chofer (id_chofer, nombre, apellido, dni, id_sucursal, id_categoria_lic)
...
-- Insert válido en la tabla camion_chofer (No se especifica fecha_asignacion, debe tomar la fecha actual por defecto)
INSERT INTO camion_chofer (fecha_asignacion, id_camion, id_chofer)
VALUES (DEFAULT, 2, 1); 
-- Se ejecuta la restricción DF_camion_chofer_fecha_asignacion
